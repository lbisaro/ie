...
class BotBaseLong(BotBase):
    ...
    """
    def backtest(self,klines,start,end,to_get='ind'):
        self.reset_res()
        self.reset_pos()
        self.backtesting = True
        ...
        self.klines = self.signal(self.klines)
        #quitando las velas previas a la fecha de start
        self.klines = self.klines[self.klines['datetime'] >= pd.to_datetime(start)]
        self.klines = self.klines.reset_index(drop=True)
        
        velas = int(self.klines['datetime'].count())
        from_date = self.klines.iloc[0]['datetime'].strftime('%Y-%m-%d')
        to_date = self.klines.iloc[-1]['datetime'].strftime('%Y-%m-%d')

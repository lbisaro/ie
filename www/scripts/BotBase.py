...
class BotBase():
    ...
    def close_pos(self,dt,price,flag):
        ...
        self.pos['result_perc'] = round( ( ( sell/(buy+comision) ) - 1 ) * 100 , 2)
        trade = [
                self.pos['start'],
                self.pos['buy_price'],
                self.pos['qty'],
    ...
    def get_brief(self):
        ...
        df_trades = pd.DataFrame(self.res['trades'], 
                                 columns = ['start','buy_price','qty','end','sell_price','flag','days','result_usd','result_perc','mea','mef'])
    ...
    def ind_indice_modificacion_sharpe(self, df_data, risk_free_rate=0.045):
